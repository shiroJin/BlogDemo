    ...
    for (int i = 0; i < CFArrayGetCount(lines); i++) {
        ...
        for (int j = 0; j < CFArrayGetCount(runs); j++) {
            ...
            if (imageName) {
                ...
                //程序包加载
                NSArray *arr = [NSArray arrayWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"emotions" ofType:@"plist"]];
                for (NSDictionary *dic in arr) {
                    if ([dic[@"chs"] isEqualToString:imageName]) {
                ...
                //沙盒加载
                if (!image) {
                    image = [UIImage imageWithData:[NSData dataWithContentsOfFile:[NSHomeDirectory() stringByAppendingPathComponent:[NSString stringWithFormat:@"documents/%@", imageName]]]];
                }
...
- (NSArray *)imagesOfRegexStrArray
{
    ...
    }//注意这个代理方法。如果没有写这个代理方法。就依照上面的regex
